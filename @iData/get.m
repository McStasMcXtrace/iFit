...
% calls: get, subsref (through s.field eval), getalias (all), getaxis (all)
...
for index = 1:length(a_in(:)) % works with object arrays
  ...
  if nargin == 1   
    ...
    for i=1:length(varargin)
      ...
      if strcmpi(fieldname, 'alias') || strncmpi(fieldname, 'alias.', 6)
        varout{argout,index} = getalias(s);
        argout = argout + 1;
        % warning('iData/get: the Alias field name is protected. Use setalias/getalias to handle Aliases.');
        continue;
      end
      if strcmpi(fieldname, 'axis') || strncmpi(fieldname, 'axis.', 5)
        varout{argout,index} = getaxis(s);
        argout = argout + 1;
      ...
      if field_not_found  % searches within all aliases
        ...
        if field_not_found
          ...
          if ~isempty(strfind(alias_num,fieldname))
            alias_num   = find(strcmpi(fieldname, alias_names));
          ...
          elseif ~isempty(find(fieldname == '.', 1)) % link may be a structure type
            [fieldroot,fieldsub] = strtok(fieldname, '.');
            try
              val = get(s, fieldroot); % gets the Alias root value
              fieldsub = textscan(fieldsub(2:end),'%s','delimiter','.');
      ...
      if field_not_found  % searches within iData fields
        ...
        if ~isempty(strfind(cell_num,fieldname))        % was the property found immediately (not a sub-field) ?
          cell_num = find(strcmpi(fieldname, names));
...
if strcmp(name, 'Error')  % Error is sqrt(Signal) if not defined
  ...
  if length(val) ~= 1 && ~isequal(size(val),size(this))
