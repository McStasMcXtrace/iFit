function signal=pseudovoigt2d(p, x, y, signal)
...
% See also iData, ifitmakefunc, gauss, iData/fits
  ...
  %   identify: model([],x,y)
    info = identify; 
    info.Guess  = [ 1 mean(x(:)) mean(y(:)) std(x(:)) std(y(:)) 30 0 0.5 ];
    info.Axes   = { x y };
...
%                                        end of model main pseudovoigt2d
% ------------------------------------------------------------------------------
...
function signal =identify()
  ...
  parameter_names = {  'Amplitude' 'Centre_X' 'Center_Y' 'HalfWidth_X' 'HalfWidth_Y' 'Angle' 'Background' 'LorentzianRatio' };
  %
  signal.Type           = 'iFit fitting function';
  signal.Name           = [ '2D Pseudo Voigt function with tilt angle (2D) [' mfilename ']' ];
...
function info=guess(x,y,signal)
  ...
  info.Guess = [ max(signal)-min(signal) mean(x) mean(y) std(x) std(y) 0 min(signal) 0.5 ];
  info.Values= evaluate(info.Guess, info.Axes{:});
end
