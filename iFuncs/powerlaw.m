...
% ex:     y=powerlaw([1 0 1 1], -10:10); or y=powerlaw('identify') or p=powerlaw('guess',x,y);
%
...
function y =identify()
  ...
  parameter_names = {'Amplitude','Centre','Exponent','Background'};
  %
  y.Type           = 'iFit fitting function';
  y.Name           = [ 'powerlaw (1D) [' mfilename ']' ];
...
function info=guess(x,y)
  ...
  % ln y = ln a + c*ln (x-b)
  g = getfield(strline('guess', log(x),log(y)), 'Guess'); % slope=c, bck=log(a)
  info.Guess = [ exp(g(1)) mean(x) g(2) min(y) ];
