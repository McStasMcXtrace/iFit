...
% ex:     y=sigmoid([1 0 1 1], -10:10); or y=sigmoid('identify') or p=sigmoid('guess',x,y);
%
...
function info=guess(x,y)
  ...
  info.Guess = [ max(y)-min(y) mean(x) std(x)/3 min(y) ];
