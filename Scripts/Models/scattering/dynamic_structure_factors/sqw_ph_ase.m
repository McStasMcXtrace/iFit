...
% start python --------------------------
script = { ...
  '# python script built by ifit.mccode.org/Models.html sqw_ph_ase', ...
[ '# on ' datestr(now) ], ...
  ...
  '# Launch with: python sqw_ph_ase_build.py (and wait...)', ...
  decl, ...
...
[ '  fprintf(fid, ''# Script file for Python/ASE to compute the modes from ' configuration ' in %s\n'', target);' ], ...
  ...
  '  fprintf(fid, ''#   <https://wiki.fysik.dtu.dk/ase>. Built by ifit.mccode.org/Models.html sqw_ph_ase'');', ...
