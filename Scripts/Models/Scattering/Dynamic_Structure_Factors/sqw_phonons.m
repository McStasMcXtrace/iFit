...
if ~isempty(options.optimizer)
  ...
  optim = sprintf([ 'from ase.optimize import %s; atoms.set_calculator(calc); ' ...
         ...
         'write("%s", atoms, "vasp");' ...
            ...
            fullfile(target, 'optimized.cif'), fullfile(target, 'optimized.pdb'), ...
            fullfile(target, 'optimized_POSCAR'));
else
  optim = '';
end
...
if strcmpi(options.calculator, 'QUANTUMESPRESSO') 
  ...
  % also does the QE case after optimisation
...
end % QE case launched in sqw_phonons_calc
