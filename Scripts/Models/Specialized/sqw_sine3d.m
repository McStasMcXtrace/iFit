...
% model = sqw_sine3d(p, h,k,l,w, {signal}) : sine dispersion(HKL) with DHO(energy) Sqw4D
%
%   iFunc/sqw_sine3d: a 4D S(q,w) sine dispersion, and a DHO line
...
y.Guess     = @(x,y,z,t,signal) iif(...
  ~isempty(signal)&&numel(signal)==numel(t), @() [ ...
    m2(t,signal) m2(t,signal) m2(t,signal) ...
    std(t(:)) round(mean(x(:))) round(mean(y(:))) round(mean(z(:))) ...
    1 1 1 std(t(:))/100 50 1 0 ], ...
  true,             @() [ 4 6 8 0  0 0 0  .5 .5 .5  0.04  50  1 0 ]);
