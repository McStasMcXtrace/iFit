...
%      sqw_linquad(0)                                 % minimum is E0=0 at q=0
%   To define an optical mode with energy E0 at Q=0, use:
%      sqw_linquad([ E0 ])                            % minimum is E0 at q=0
...
%      sqw_linquad(E0)           centers the excitation at q=0 with energy E0
%      sqw_linquad([ h k l E0 ]) centers the excitation at q=[H K L] and energy E0
...
%             p(17)= Background   
%             p(18)= Ex_E1_Center     Linear dependency energy center [meV]
%          or p='guess'
...
'dvx = (dx*U1(1) +dy*U1(2) +dz*U1(3) )*p(7)+p(18); % meV2, lin', ...
'dvx = dvx.^2;', ...
...
elseif nargin == 1 && isnumeric(varargin{1})
  if length(varargin{1}) == 1 % [ Emax ]
    p(13) = varargin{1}; 
    p(18) = 0; fix(signal,'Ex_E1_Center'); 
    munlock(signal, 'Ex_E0_Center');
  elseif length(varargin{1}) == 4 % [HKLE]
    p(10:13) = varargin{1};
    % [ 0 0 0 E0 ] optical
    % [ h k l E0 ] acoustic
    if all(p(10:13) == 0), p(18)=0; fix(signal,'Ex_E1_Center'); munlock(signal, 'Ex_E0_Center');
    else p(18)=p(14);      p(14)=0; fix(signal,'Ex_E0_Center'); munlock(signal, 'Ex_E1_Center');
    end
  end
  signal.ParameterValues = p;
elseif nargin > 1
