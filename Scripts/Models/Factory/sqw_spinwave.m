...
template = regexprep(template, [ tok{1} '=\w+\.?\w*,?' ], '');
...
s.Expression = { ...
  ...
  '% replace tokens as variable parameters in the template' ...
  'template = this.UserData.template;' ...
  'for index=1:numel(this.Parameters)' ...
  '  template = strrep(template, [ ''%'' this.Parameters{index} ], p(index));' ...
  '  template = strrep(template, [ ''$'' this.Parameters{index} ], p(index));' ...
  'end' ...
  ...
  'signal = zeros([ numel(xu) numel(yu) numel(zu) numel(tu) ];' ...
  'for ie=1:numel(tu)' ...
    'templateE = [ template sprintf(''COUPE,EN0=%g,NP=%i\n'', tu(i1), numel(xu)) ];' ...
    'volHKL = zeros(sz);' ...
    'for is=1:numel(ax{L_id})' ...
      ...
      '  disp([ ''model '' this.Name '' '' this.Tag '' could not run SPINWAVE from '' target ]); disp(getReport(ME)); return', ...
      'end', ...
      'cd(pw)' ...
      '% get result and catenate' ...
      'cut2D = load(''results.dat'');' ...
