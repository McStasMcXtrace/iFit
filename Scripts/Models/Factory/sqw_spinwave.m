...
s.Expression = { ...
  ...
  'if all(cellfun(@(c)max(size(c)) == numel(c), {x y z})) && numel(unique(sz)) > 1 && 0', ...
    '% QE grid' ...
    [ 'disp([ ''SpinWave "QE" EN='' mat2str([ min(tu), max(tu) ]) ' ...
      ''' QX='' num2str(mean(xu)) ' ...
      ''' QY='' num2str(mean(yu)) ' ...
      ''' QZ='' num2str(mean(zu)) ])' ] ... 
    ...
    '% execute: write the spinwave input file' ...
    'try' ...
      'fid = fopen(fullfile(target, ''input.txt''), ''w'');' ...
      'fprintf(fid, ''%s'', templateE);' ...
      'fclose(fid)' ...
      ...
      'signal = load(fullfile(target, ''results.dat''),''-ascii'');' ...
      'signal = signal(:,5);' ...
      'signal = reshape(signal, max(sz), numel(tu));' ...
    'catch ME; this.UserData.log = fileread(fullfile(target,''spinwave.log'')); ', ...
  ...
  'else % volume' ...
    ...
    'for ie = 1:numel(tu)' ...
      ...
      'for  is=1:numel(ax{L_id})' ...
        '% starting point' ...
        'Qs = ax{L_id};' ...
        [ 'disp([ ''SpinWave "HKLE" EN='' num2str(tu(ie)) ' ...
          ''' Q'' N_id(L_id) ''='' num2str(Qs(is)) ' ...
          ''' Q'' N_id(G_id(1)) ''='' mat2str([min(ax{G_id(1)}) max(ax{G_id(1)}) ]) ' ...
          ''' Q'' N_id(G_id(2)) ''='' mat2str([min(ax{G_id(2)}) max(ax{G_id(2)}) ]) ])' ] ... 
