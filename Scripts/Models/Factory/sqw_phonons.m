...
if ~strcmpi(options.calculator, 'QUANTUMESPRESSO') || strcmpi(options.calculator, 'QUANTUMESPRESSO_ASE')
  ...
  [ '  [status,result] = system([ ''' precmd 'python '' fullfile(target, ''sqw_phonons_eval.py'') ]);' ], ...
    ...
    '   entropy.Title=[ ''Entropy S=-dF/dT [eV] '' strtok(this.Name) ]; xlabel(entropy,''Temperature [K]'');', ...
    '   this.UserData.entropy = entropy;', ...
    '   helmholtz_energy = iData(this.UserData.properties.temperatures_thermo, this.UserData.properties.helmholtz_energy);', ...
    '   helmholtz_energy.Title=[ ''Helmholtz energy F=U-TS=-kT lnZ [eV] '' strtok(this.Name) ]; xlabel(helmholtz_energy,''Temperature [K]'');', ...
    '   this.UserData.helmholtz_energy = helmholtz_energy;', ...
    '   internal_energy = iData(this.UserData.properties.temperatures_thermo, this.UserData.properties.internal_energy);', ...
    '   internal_energy.Title=[ ''Internal energy U [eV] '' strtok(this.Name) ]; xlabel(internal_energy,''Temperature [K]'');', ...
    '   this.UserData.internal_energy = internal_energy;', ...
    '   Cv = diff(internal_energy);', ...
    '   Cv.Title=[ ''Specific heat at constant volume Cv=dU/dT [eV/K] '' strtok(this.Name) ]; xlabel(Cv,''Temperature [K]'');', ...
    '   this.UserData.heat_capacity = Cv;', ...
    ' end', ...
