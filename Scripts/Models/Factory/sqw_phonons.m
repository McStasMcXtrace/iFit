...
if ~strcmpi(options.calculator, 'QUANTUMESPRESSO') || strcmpi(options.calculator, 'QUANTUMESPRESSO_ASE')
  ...
  [ '  fprintf(fid, ''# Script file for Python/ASE to compute the modes from ' configuration ' in %s\n'', target);' ], ...
    ...
    '  fprintf(fid, ''sio.savemat("FREQ.mat", { "FREQ": omega_kn, "POLAR":polar_kn })\n'');', ...
  ...
  [ '  [status,result] = system([ ''' precmd 'python '' fullfile(target, ''sqw_phonons_eval.py'') ]);' ], ...
    ...
    '  else disp(result); disp(''...''); end', ...
    '  clear HKL', ...
    '  % import FREQ', ...
    '  FREQ=load(fullfile(target,''FREQ.mat'')); % in meV', ...
    '  POLAR=FREQ.POLAR; FREQ=FREQ.FREQ;', ...
    '  neg_w = find(FREQ(:) <= 0 | ~isreal(FREQ(:)));', ...
    '  if ~isempty(neg_w), disp([ ''WARNING: found '' num2str(numel(neg_w)) '' negative/imaginary phonon frequencies ('' num2str(numel(neg_w)*100/numel(FREQ)) ''% of total)'' ]); end', ...
    ...
    '% we may evaluate the phonon intensity with FREQ and POLAR, but need the scattering', ...
    '% length per atom in the cell: ', ...
    '%   I ~ sum (HKL .* POLAR(index))^2/FREQ(index)*(n(FREQ(index)))', ...
    'for index=1:size(FREQ,2) % iteration per mode', ...
