...
%   The model includes the Debye-Waller structure factor, and satisfies the 
%     detailed balance. Pure inelastic coherent 4D model.
%
...
script_compute_HKL = { ...
    '  if ~this.UserData.options.use_phonopy || isempty(this.UserData.options.available.phonopy) || isempty(dir(fullfile(target, ''phonopy.pkl'')))', ...
...
[ '  [status,result] = system([ ''' precmd ' ' options.available.python ' '' fullfile(target, ''sqw_phonons_eval.py'') ]);' ], ...
