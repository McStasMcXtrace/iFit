...
%
% 'dos': will also compute the vibrational density of states and thermo-chemistry.
...
%     density of states (vDOS) in UserData.DOS. The Thermo-Chemistry quantities
%     U,S,F and Cv are also computed.
...
if ~strcmpi(options.calculator, 'QUANTUMESPRESSO') || strcmpi(options.calculator, 'QUANTUMESPRESSO_ASE')
  ...
  if ~isempty(dir(fullfile(target, 'phonopy.pkl')))
    signal.UserData.phonons = fileread(fullfile(target, 'phonopy.pkl')); % binary
  end
  ...
  if ~options.use_phonopy
    ...
    '  fprintf(fid, ''omega_e *= 1000.0\n'');' };
  else
    disp('Using PhonoPy for computation and evaluation with ASE calculator.')
    % strangely, PhonoPy requires an additional factor pi for conversion to meV ??!!
    script_compute_HKL = { ...
      '  fprintf(fid, ''fid = open("phonopy.pkl", "rb")\n'');', ...
      '  fprintf(fid, ''ph = pickle.load(fid)\n'');', ...
      '  fprintf(fid, ''fid.close()\n'');', ...
      '  fprintf(fid, ''omega_kn = []\n'');', ...
      '  fprintf(fid, ''polar_kn = 0.0\n'');', ...
      '  fprintf(fid, ''for q in enumerate(HKL):\n'');', ...
      '  fprintf(fid, ''    omega_kn.append(ph.get_frequencies(q[1]))\n'');', ...
      '  fprintf(fid, ''    \n'');', ...
      '  fprintf(fid, ''omega_kn = numpy.asarray(omega_kn)\n'');', ...
      '  fprintf(fid, ''omega_kn = numpy.sqrt(abs(omega_kn)) * numpy.sign(omega_kn)\n'');', ...
      '  fprintf(fid, ''omega_kn *= 4.13*numpy.pi\n'');' ...
    };
    script_compute_DOS = { ...
      '  fprintf(fid, ''omega_e=0.0\ndos_e=0.0\n\n'');', ...
    };
  end
  ...
  [ '  fprintf(fid, ''# Script file for Python/ASE to compute the modes from ' configuration ' in %s\n'', target);' ], ...
    ...
    '  fprintf(fid, ''from ase.thermochemistry import CrystalThermo\n'');', ...
    ...
    '  fprintf(fid, ''print "Computing crystal ThermoChemistry for T=0-500 [K]\\n"\n'');', ...
    '  fprintf(fid, ''# read properties from pickle\n'');', ...
    '  fprintf(fid, ''fid = open("properties.pkl","rb")\n'');', ...
    '  fprintf(fid, ''properties = pickle.load(fid) # and get potential_energy\n'');', ...
    '  fprintf(fid, ''fid.close()\n'');', ...
    '  fprintf(fid, ''try:    \n'');', ...
    '  fprintf(fid, ''    try:    # ASE <= 3.9\n'');', ...
    '  fprintf(fid, ''        thermo = CrystalThermo(phonon_energies=omega_e/1000.0, phonon_DOS=dos_e, electronicenergy=properties["potential_energy"])\n'');', ...
    '  fprintf(fid, ''    except: # ASE >= 3.10\n'');', ...
    '  fprintf(fid, ''        thermo = CrystalThermo(phonon_energies=omega_e/1000.0, phonon_DOS=dos_e, potentialenergy=properties["potential_energy"])\n'');', ...
    '  fprintf(fid, ''    T = numpy.arange(1,500)\n'');', ...
    '  fprintf(fid, ''    S = [ thermo.get_entropy(temperature=x) for x in T]\n'');', ...
    '  fprintf(fid, ''    F = [ thermo.get_helmholtz_energy(temperature=x) for x in T]\n'');', ...
    '  fprintf(fid, ''    U = [ thermo.get_internal_energy(temperature=x) for x in T]\n'');', ...
    '  fprintf(fid, ''    properties["entropy"] = S           # in eV/K\n'');', ...
    '  fprintf(fid, ''    properties["helmholtz_energy"] = F  # in eV\n'');', ...
    '  fprintf(fid, ''    properties["internal_energy"] = U   # in eV\n'');', ...
    '  fprintf(fid, ''    properties["temperatures_thermo"] = T   # in K\n'');', ...
    '  fprintf(fid, ''    fid = open("properties.pkl","wb")\n'');', ...
    '  fprintf(fid, ''    pickle.dump(properties, fid)\n'');', ...
    '  fprintf(fid, ''    fid.close()\n'');', ...
    '  fprintf(fid, ''except:    \n'');', ...
    '  fprintf(fid, ''    print "Skipped ThermoChemistry."\n'');', ...
    '  fprintf(fid, ''# export properties as MAT\n'');', ...
    '  fprintf(fid, ''sio.savemat("properties.mat", properties)\n'');', ...
    'end % compute DOS and Thermo stuff', ...
    '  fprintf(fid, ''exit()\n'');', ...
    '  fclose(fid);', ...
    script_hkl{:}, ...
  ...
  [ '  [status,result] = system([ ''' precmd 'python '' fullfile(target, ''sqw_phonons_eval.py'') ]);' ], ...
    ...
    'catch ME; disp([ ''Model '' this.Name '' '' this.Tag '' could not run Python/ASE from '' target ]); whos; disp(getReport(ME));', ...
    'end', ...
    ...
    '   properties = load(fullfile(target,''properties.mat''))', ...
    '   this.UserData.properties = rmfield(this.UserData.properties, intersect(fieldnames(this.UserData.properties), fieldnames(properties)));', ...
    ...
    '   if isfield(this.UserData.properties,''temperatures_thermo'')', ...
    ...
    '     helmholtz_energy.Error=0; this.UserData.helmholtz_energy = helmholtz_energy;', ...
    '     internal_energy = iData(this.UserData.properties.temperatures_thermo, this.UserData.properties.internal_energy);', ...
    '     internal_energy.Title=[ ''Internal energy U [eV/cell] '' strtok(this.Name) ]; xlabel(internal_energy,''Temperature [K]''); ylabel(internal_energy,''U [eV/cell]'');', ...
    '     internal_energy.Error=0; this.UserData.internal_energy = internal_energy;', ...
    '     Cv = diff(internal_energy);', ...
    '     Cv.Title=[ ''Specific heat at constant volume Cv=dU/dT [eV/K/cell] '' strtok(this.Name) ]; xlabel(Cv,''Temperature [K]''); ylabel(Cv, ''Cv [eV/K/cell]'');', ...
    '     Cv.Error=0; this.UserData.heat_capacity = Cv;', ...
    '   end', ...
    ' end', ...
    'catch ME; disp([ ''Model '' this.Name '' '' this.Tag '' could not get vDOS/ThermoChemistry Python/ASE from '' target ]); whos; disp(getReport(ME));', ...
    'end', ...
...
if isfield(options, 'dos') && options.dos && ~strcmpi(options.calculator, 'QUANTUMESPRESSO')
  disp('INFO: The vibrational density of states (vDOS) and thermo chemistry quantities will be computed at first model evaluation.');
end
