...
if ~strcmpi(options.calculator, 'QUANTUMESPRESSO') || strcmpi(options.calculator, 'QUANTUMESPRESSO_ASE')
  ...
  if ~options.use_phonopy || 1
    ...
    script_compute_HKL = { ...
      ...
      '  fprintf(fid, ''polar_kn = 0\n'');', ...
      '  fprintf(fid, ''for i, q in enumerate(HKL):\n'');', ...
      '  fprintf(fid, ''    omega_kn.append(ph.get_frequencies(q))\n'');', ...
      '  fprintf(fid, ''    \n'');', ...
      '  fprintf(fid, ''omega_kn = numpy.asarray(numpy.sqrt(abs(omega_kn)) * numpy.sign(omega_kn), dtype="double",order="C")\n'');', ...
    };
    script_compute_DOS = { ...
  ...
  [ '  fprintf(fid, ''# Script file for Python/ASE to compute the modes from ' configuration ' in %s\n'', target);' ], ...
    ...
    '  fprintf(fid, ''    fid.close()\n'');', ...
    '  fprintf(fid, ''except:    \n'');', ...
    '  fprintf(fid, ''    print "Failed ThermoChemistry"\n'');', ...
