...
if ishandle(options.gui), 
  waitbar(0.05, options.gui, [ mfilename ': configuring ' options.calculator ]); 
end
result = '';
disp([ mfilename ': reading material structure in ' target ' (sqw_phonons_check.py)' ]);
try
...
if ~isempty(options.optimizer) && ...
  ...
  if fid ~= -1
    ...
    % ------------------------------------------------------------------------------
    if ishandle(options.gui), 
      waitbar(0.10, options.gui, [ mfilename ': optimizing ' options.calculator ]); 
    end
    result = '';
    disp([ mfilename ': optimize material structure in ' target ' (sqw_phonons_optimize.py)' ]);
    try
...
if ~strcmpi(options.calculator, 'QUANTUMESPRESSO') || strcmpi(options.calculator, 'QUANTUMESPRESSO_ASE')
  ...
  options.script_get_forces = [ ...
    ...
    '# S. R. Bahn and K. W. Jacobsen, Comput. Sci. Eng., Vol. 4, 56-66, 2002.\n', ...
    '#\n', ...
    '# Computes the forces and stores an ase.phonon.Phonons object in a pickle ph.pkl\n', ...
  ...
  if ishandle(options.gui), waitbar(0.15, options.gui, [ mfilename ': computing Forces ASE/' options.calculator ' (be patient)' ]); end
  ...
  try
    if strcmpi(options.calculator, 'GPAW') && isfield(options,'mpi') ...
      ...
      [st, result] = system([ precmd status.mpirun ' -np ' num2str(options.mpi) ' '  status.gpaw ' sqw_phonons_forces.py' ]);
    else
      [st, result] = system([ precmd 'python sqw_phonons_forces.py' ]);
    end
    disp(result)
  catch
    disp(result)
