...
if ~strcmpi(options.calculator, 'QUANTUMESPRESSO') || strcmpi(options.calculator, 'QUANTUMESPRESSO_ASE')
  ...
  if ~options.use_phonopy || 1
    ...
    script_compute_HKL = { ...
      ...
      '  fprintf(fid, ''ph = pickle.load(fid)\n'');', ...
      '  fprintf(fid, ''fid.close()\n'');', ...
      '  fprintf(fid, ''num_band = ph._unitcell.get_number_of_atoms() * 3\n'');', ...
      '  fprintf(fid, ''num_qpoints = len(HKL)\n'');', ...
      '  fprintf(fid, ''omega_kn = numpy.zeros((num_qpoints, num_band), dtype="double")\n'');', ...
      '  fprintf(fid, ''polar_kn = numpy.zeros((num_qpoints, num_band, num_band,),dtype="complex128")\n'');', ...
      '  fprintf(fid, ''for i, q in enumerate(HKL):\n'');', ...
      '  fprintf(fid, ''    ph._dynamical_matrix.set_dynamical_matrix(q)\n'');', ...
      '  fprintf(fid, ''    dm = ph._dynamical_matrix.get_dynamical_matrix()\n'');', ...
      '  fprintf(fid, ''    eigvals, polar_kn[i] = numpy.linalg.eigh(dm)\n'');', ...
      '  fprintf(fid, ''    omega_kn[i] = eigvals.real\n'');', ...
      '  fprintf(fid, ''    \n'');', ...
      '  fprintf(fid, ''omega_kn = numpy.array(numpy.sqrt(abs(omega_kn)) * numpy.sign(omega_kn), dtype="double",order="C")\n'');', ...
    };
    script_compute_DOS = { ...
      '  fprintf(fid, ''omega_e=0\ndos_e=0\n\n'');', ...
    };
  end
  ...
  [ '  fprintf(fid, ''# Script file for Python/ASE to compute the modes from ' configuration ' in %s\n'', target);' ], ...
    ...
    '  fprintf(fid, ''properties["entropy"] = S           # in eV\n'');', ...
  ...
  [ '  [status,result] = system([ ''' precmd 'python '' fullfile(target, ''sqw_phonons_eval.py'') ]);' ], ...
    ...
    ' end', ...
    'catch ME; disp([ ''Model '' this.Name '' '' this.Tag '' could not get vDOS/ThermoChemistry Python/ASE from '' target ]); disp(getReport(ME));', ...
    'end', ...
