...
% Octopus requires to keep the initial lattice orthogonal.
% reducing to the primitive does not help in this case.
if ~strcmpi(options.calculator, 'octopus')
  primitive = '    atoms1 = ifit.find_primitive(atoms)\n';
else
  primitive = '    atoms1 = None\n';
end
...
result = '';
cmd=[ precmd options.available.python ' ' fullfile(target,'sqw_phonons_check.py') ];
disp(cmd);
try
