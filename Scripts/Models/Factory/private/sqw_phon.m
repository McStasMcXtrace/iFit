...
[ '  fprintf(fid, ''# number of ion types and masses\n  NTYPES = ' num2str(numel(geom.symbols)) '\n  MASS = ' mass '\n'');' ], ...  
  ...
  'if all(cellfun(@isvector, {x y z})) && numel(unique(cellfun(@(x)length(x), {x y z}))) > 1', ...
  '  fprintf(fid, ''  ND=%i\n'', numel(x));', ...
...
[ '  [status,result] = system(''' precmd options.phon ' > phon.log'');' ], ...
  ...
  'end', ...
  '  delete(''FREQ.cm'');', ...
  'catch; disp(fileread(''phon.log'')); disp([ ''model '' this.Name '' '' this.Tag '' could not run PHON from '' target ]); return', ...
  'end', ...
  '  cd(pw);', ...
...
if isempty(dir(fullfile(p,'FORCES')))
  ...
  for move=1:size(displacements,1)
    ...
    elseif move < size(displacements,1)
      ...
      remaining = seconds(toc(t)/move*(size(displacements,1)-move));
      disp([ mfilename ': ETA ' char(remaining) ', ending on ' char(t0+remaining) ]);
    end
