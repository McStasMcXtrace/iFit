...
% ex:     y=bilorz([1 0 1 1], -10:10); or y=bilorz('identify') or p=bilorz('guess',x,y);
%
...
function y = evaluate(p, x)
  ...
  x1 = x(find(x <  p(2)));
  x2 = x(find(x >= p(2)));
  y1 = p(1) ./ (1+ ((x1-p(2)).^2/p(3)^2) ) + p(5);
  y2 = p(1) ./ (1+ ((x2-p(2)).^2/p(4)^2) ) + p(5);
  y = [ y1 ; y2 ]; 
