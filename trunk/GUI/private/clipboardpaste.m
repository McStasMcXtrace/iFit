function clipboardData = clipboardpaste(format)
    ...
    %   Updated by E. Farhi on Jan 12 2009 to handle java.awt.image as generated from im2java
    %
    %   See also clipboard, clipboardcopy.
...
function clipboardData = getImageFromClipboardData(imarr,format)
    ...
    theImage = imarr; % usually of type BufferedImage
    if strmatch('BufferedImage', char(theImage));
      if ismac
        try
      ...
      % theImage must be a Rendered image type
      javax.imageio.ImageIO.write(theImage,format,filehandle);
      clipboardData = imread(filename);
      delete(filename);
    else  % may also be a ToolkitImage
      % obtained from Matlab Solution ID 1-2WPAYR: How can I convert a "Java Image" object into a MATLAB image matrix?
      H=theImage.getHeight;
      W=theImage.getWidth;

      % get the image data
      B=theImage.getBufferedImage.getData.getPixels(0,0,W,H,[]);
      C=uint8(zeros([H,W,3]));
      bidx = 1;
      for i=1:H
      for j=1:W
