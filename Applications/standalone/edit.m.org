...
if isnumeric(url) && ishandle(url) && feature('ShowFigureWindows')
  ...
  elseif ismac
    ...
    if isempty(editor) || any(strcmp(editor,{'vi','vim','nano'}))
      ...
      if feature('ShowFigureWindows')
        % test editors that require X display
        ed={'gedit','nedit','kate','xedit','xemacs'};
        for index=ed
          [flag, out]=system(['locate ' ed{1} ]);
          if isempty(editor) && ~isempty(out), editor=ed{1}; break; end
        end
      else
        % test editors that do not require X display
        ed={'nano','less','more'};
        for index=ed
          [flag, out]=system(['locate ' ed{1} ]);
          if isempty(editor) && ~isempty(out), editor=ed{1}; break; end
...
if success ~=0
  TextEdit(url); % from Apps/contrib
end
