...
% the select argument can be 0 or 1 to deselect/select all
  ...
  if (numel(index_selected) == numel(items) && isempty(select)) ...
    || (~isempty(select) && isscalar(select) && ~select), index_selected = [];
  elseif isempty(select) || (~isempty(select) && isscalar(select) && select), index_selected=1:numel(items); 
  elseif numel(select) > 1 && isnumeric(select) && min(select) > 0 && max(select) <= numel(items)
