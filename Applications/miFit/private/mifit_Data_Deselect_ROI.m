function mifit_Data_Deselect_ROI(varargin)
% Data/Deselect_ROI: deselect a ROI, then create a new selected data set, and its mask as [0-1]
  ...
  mifit_disp([ '[Data select ROI] Deselecting ROI on ' char(D{1}) ]);
  set(mifit_fig,'Pointer','watch');
  
  [this, mask, f] = imroi(D{1});
  close(f);
  mask = ~mask; % invert mask (deselect)
