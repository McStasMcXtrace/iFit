...
for index=1:length(varargin)
  ...
  elseif isa(varargin{index}, 'estruct')  % object axes
    if length(c) > 1
      warning([ mfilename ': Can not interpolate onto all axes of input argument ' num2str(index) ' which is an array of ' num2str(numel(c)) ' elements. Using first element only.']);
      c = c(1);
    end
    for j1 = 1:ndims(c)
...
if length(f_axes) > 1 && (requires_meshgrid || ntimes)
  f_axes = private_meshgrid(f_axes, s_dims, method); % private function
end
