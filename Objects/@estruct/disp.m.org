...
if numel(s_in) > 1
  ...
  % update title
  T   = s.Name; if ~ischar(T), T=char(T); end
  ...
  for index=1:length(s_in.Alias.Names)
    ...
    if (isnumeric(v) | islogical(v)) && ~isa(v, 'es') && ~isa(v, 'iFunc'),
    ...
    elseif isobject(v) && ismethod(v, 'char')
      v=char(v);
    elseif isa(v,'iFunc')
      v=[ 'iFunc ' v.Name ];
    else
      ...
      try   % use matlab 'tostring' by capturing the display
        ...
        v=class2str('s',v,'no comments, short');
      end
    ...
    if strcmp(s_in.Alias.Names{index}, 'Format') && ~isdeployed && nargin < 3
      ...
      label=[ '<a href="matlab:doc(estruct,''Load'')">' label '</a>' ];
    end
  ...
  for index=0:length(s_in.Alias.Axis)
    ...
    if index==0
      ...
      minmaxstd=[ minmaxstd sprintf(' sum=%g', sum(estruct_private_cleannaninf(x))) ];
    end
