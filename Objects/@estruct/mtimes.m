...
elseif ndims(a) == 2 & ndims(b) == 2
  ...
  if size(a,2) ~= size(b,1)
    estruct_private_error(mfilename,[ 'the number of columns of a (' num2str(size(a,2)) ') must equal the number of rows of b (' num2str(size(b,1)) ').' ]);
  end
  cmd=a.Command;
  c = copyobj(a);
  ...
  c.Command=cmd;
  c = estruct_private_history(c, mfilename, a,b);
else
  estruct_private_error(mfilename,[ 'Matrix estruct multiplication only supported for matrix estruct object (ndims=2), and not current ndims(a)=' num2str(ndims(a)) ' and ndims(b)=' num2str(ndims(b)) '.' ]);
end
