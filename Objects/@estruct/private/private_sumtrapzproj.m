function [s, sigma] = private_sumtrapzproj(a,dim, op)
% s = private_sumtrapzproj(a,dim, op) : computes the sum/trapz/camproj of estruct objects elements
%
%   @estruct/private_sumtrapzproj function to compute the sum/trapz/camproj of the elements of the data set
%
% input:  a: object or array (estruct/array of)
%         dim: dimension to accumulate (int/array of/'radial')
%         op: 'sum','trapz','camproj','prod'
% output: s: sum/trapz/camproj of elements (estruct/scalar)
% ex:     c=private_sumtrapzproj(a, dim, 'sum');
%
% Version: $Date$ $Version$ $Author$
% See also estruct, estruct/plus, estruct/prod, estruct/cumsum, estruct/mean, estruct/camproj, estruct/trapz
...
if isempty(s) || ~isnumeric(s), if dim, s=estruct; else s=0; end; return; end
e = get(a,'Error');   % raw Error  (no Monitor weight)
m = private_cleannaninf(get(a,'Monitor'));
...
if all(dim > 0)
  ...
  switch op
  case {'sum','cumsum'} % SUM ==================================================
    % sum on all requested dimensions 
