function filename = private_saveas_html(a, filename, format)
  % save an estruct into an HTML document
  ...
  % object description
  data = [];
  if ~isempty(a.Name) || ~isempty(title(a))
    data.Title  = strtrim([ a.Name ' ' title(a) ]); end
 ...
 fprintf(fid, '<p>Exported to: <br><ul>\n');
  ...
  if strcmp(mode, 'w+')
    ...
    fprintf(fid,[ '<b>' datestr(now) '</b> - ' version(estruct) '<br>\n' ]);
