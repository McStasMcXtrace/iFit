function filename = private_saveas_dat(a, filename)
  NL = sprintf('\n');
  str = [ '# Format: data with text headers' NL ...
            '# URL: http://ifit.mccode.org' NL ...
            '# Creator: iFit/@estruct/saveas - ' version(a) NL ...
            '# Title: ' a.Name NL ...
            '# Label: ' a.Label NL ...
  ...
  if fid == -1
    estruct_private_warning(mfilename,[ 'Error opening file ' filename ' to save object ' a.Tag 'in format ' format ]);
    warning(message)
    return
  end
  fprintf(fid, '%s', str);
