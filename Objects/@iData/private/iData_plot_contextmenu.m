...
if ndims(a) == 1
  uimenu(uicm, 'Label','Toggle Error Bars', 'Callback', [...
   'h=gco; disp(''gco=''); disp(h); if strcmpi(get(get(h,''Parent''),''Type''), ''Hggroup''), h=get(h,''Parent''); end; disp(''h='');disp(h); ' ...
   'tmp_h=get(h,''children''); disp(tmp_h);  '...
   'if ~isempty(tmp_h), if strcmp(get(tmp_h(2),''visible''),''off''), tmp_v=''on''; else tmp_v=''off''; end;' ...
   'set(tmp_h(2:end),''visible'',tmp_v); end; clear tmp_h tmp_v' ]);
end
