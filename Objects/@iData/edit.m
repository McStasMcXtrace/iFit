function [h, f] = edit(a, option)
  ...
  %
  %   EDIT(iData, handle) extract values from the Table handle into an iData object.
  %
  %   [h, fig] = EDIT(...) return the Table and Figure handles.
  %
  % Example: s=iData(rand(5)); [h,f]=edit(s); tf=ishandle(h); close(f); tf
  %
...
function inline_edit_export(varargin)
  ...
  fig = gcbf;
  UserData  = get(fig, 'UserData');
  d=inline_edit_iData(UserData.handle);
...
function d=inline_edit_iData(h)
