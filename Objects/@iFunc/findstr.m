...
if iscell(str) && ischar(str{1})
  ...
  for index=1:numel(str)
    [m,f] = iFunc_findstr_single(str{index}, option, fields, matchs);
    match{index}=m;
    field{index}=f;
  end
  return
...
[match, field] = iFunc_findstr_single(str, option, fields, matchs);

% -------------------------------------------------------------------------
function [match, field] = iFunc_findstr_single(str, option, field, match)
