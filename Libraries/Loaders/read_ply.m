function data = read_ply(filename, varargin)
...
if nargin == 0 || any(strcmp(filename, {'identify','query','defaults'}))
    ...
    PLY_binary.name      ='PLY 3D binary';
    PLY_binary.method    =mfilename;
    PLY_binary.extension ='ply';
    PLY_binary.options   ='binary';
    PLY_binary.patterns  ={'ply','format binary_','element','end_header'};
    PLY_binary.postprocess='openply';
    
    data = { PLY_ascii, PLY_binary };
    return
end

% first assume we have a text based format
% now call read_anytext with given options

if isempty(varargin)
  varargin = { '--fast --binary --headers --comment=NULL --silent ' };
end
if ~isempty(varargin) && ~strcmp(varargin{1}, 'binary')
  data       = read_anytext(filename, varargin{:});
else data=[]; end
...
% then try a binary PLY reader
[data.faces,data.vertices,data.elmts,data.comments] = plyread(filename, 'tri');
return
