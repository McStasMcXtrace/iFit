...
% iLoad_config_find: searches in the config for given names
  ...
  elseif ischar(loader)
    ...
    for index=1:length(formats)
      ...
      i1 = strfind(lower(this_loader.name), lower(loader));   
      if isempty(i1), i1=0; end
      i2 = strfind(lower(this_loader.method), lower(loader)); 
      if isempty(i2), i2=0; end
      i3 = isfield(this_loader,'extension') && ...
          ~isempty(strfind(lower(this_loader.extension), lower(loader))); 
      if iscell(i3), i3 = ~cellfun('isempty', i3); end
      i4 = isfield(this_loader,'postprocess') && ...
          ~isempty(strfind(lower(this_loader.postprocess), lower(loader))); 
      if iscell(i4), i4 = ~cellfun('isempty', i4); end
